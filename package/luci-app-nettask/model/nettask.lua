...
if pid == "" then
    ...
    function detect.write()
        -- 确保日志文件存在
        if not fs.access("/var/log/nettask.log") then
            luci.sys.call("touch /var/log/nettask.log && chmod 666 /var/log/nettask.log")
        end
        luci.sys.call("sh /etc/nettask/duan.sh >> /var/log/nettask.log 2>&1 &")
        luci.http.redirect(luci.dispatcher.build_url("admin", "services", "NetTask"))
    end
else
    ...
    function detect.write()
        luci.sys.call("pgrep -f /etc/nettask/duan.sh | xargs kill -9 >> /var/log/nettask.log 2>&1")
        luci.http.redirect(luci.dispatcher.build_url("admin", "services", "NetTask"))
    end
...
function conf2.write(self, section, value)
    if value then
        ...
        if (luci.sys.call("cmp -s /tmp/word.sh /etc/nettask/word.sh") == 1) then
            ...
            luci.sys.call("sh /etc/nettask/word.sh >> /var/log/nettask.log 2>&1 &")
        end
...
s:tab("log", translate("运行日志"), translate("显示脚本运行日志"))

-- 添加日志显示区域
logview = s:taboption("log", TextValue, "logview")
...
function logview.cfgvalue(self, section)
    local logfile = "/var/log/nettask.log"
...
-- 添加刷新按钮
refresh = s:taboption("log", Button, "refresh", translate("刷新日志"))
refresh.inputstyle = "reload"
function refresh.write(self, section)
    luci.http.redirect(luci.dispatcher.build_url("admin", "services", "NetTask"))
end

-- 添加清空日志按钮 
clear = s:taboption("log", Button, "clear", translate("清空日志"))
clear.inputstyle = "remove"
function clear.write(self, section)
    luci.sys.call("echo '' > /var/log/nettask.log")
    luci.http.redirect(luci.dispatcher.build_url("admin", "services", "NetTask"))
end
